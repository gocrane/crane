...
To see if a signal has a period of $$T$$, one needs to observe at least twice the length of $$T$$, so the longest period can be identified by a sequence of length $$N$$ is $$N/2$$. So $k = 1$ can be ignored.
